# Social Media Content Analytics Dashboard (Google Looker Studio)

## 📌 Project Type
Personal Project

## 📊 Overview
Project ini merupakan dashboard interaktif berbasis Google Looker Studio yang dikembangkan untuk menganalisis performa konten media sosial pada berbagai platform digital.

Fokus utama dari project ini adalah mengolah data aktivitas media sosial menjadi informasi yang terstruktur, konsisten, dan mudah dianalisis. Proses yang dilakukan mencakup data cleaning, data transformation, pembuatan calculated fields, serta visualisasi data untuk mengidentifikasi pola performa konten secara menyeluruh.

## 🎯 Objectives
- Menyajikan ringkasan performa media sosial melalui KPI utama
- Menganalisis tren reach dan impressions dari waktu ke waktu
- Mengevaluasi distribusi engagement berdasarkan platform
- Mengidentifikasi performa berdasarkan content type dan content theme

## ⚙️ Data Processing
- Data cleaning untuk memastikan konsistensi dan kualitas data
- Penanganan missing values
- Pembuatan calculated fields
- Standardisasi kategori dan penamaan data
- Data transformation untuk mendukung visualisasi dashboard

## 🧠 Business Logic
- Reach dihitung berdasarkan jumlah akun unik yang melihat konten
- Impressions dihitung berdasarkan total tayangan konten, termasuk tayangan berulang dari pengguna yang sama
- Engagement dihitung dari akumulasi likes, comments, shares, saves, dan link clicks
- Total Ad Spend dihitung berdasarkan total biaya iklan yang dikeluarkan untuk aktivitas promosi konten
- Perbandingan KPI terhadap periode sebelumnya digunakan untuk mengevaluasi perubahan performa media sosial

## 🛠️ Tools & Skills
- Google Sheets
- Google Looker Studio
- Data cleaning
- Data transformation
- Calculated fields
- Data visualization & dashboard design
- Business analysis

## 🖼️ Preview
Berikut dashboard interaktif yang dapat dieksplorasi secara langsung melalui Google Looker Studio:
https://datastudio.google.com/reporting/6bb4125a-f782-4134-a511-834997cdce93
<img width="2500" height="3126" alt="Social Media Content Analytics" src="https://github.com/user-attachments/assets/e1a93fb3-60a0-4dd9-a5dc-0cade1df2b10" />

## 📊 Insights
- Pada periode 1–30 September 2025, aktivitas media sosial menghasilkan total reach sebesar 38.800, total impressions sebesar 96.730, total engagement sebesar 6.740, dan total ad spend sebesar Rp58,99 juta. Dibandingkan dengan 30 hari sebelumnya, reach meningkat 12,5%, impressions meningkat 22,0%, engagement meningkat 18,5%, sementara total ad spend menurun 7,9%, menunjukkan peningkatan performa dengan penggunaan anggaran yang lebih efisien.
- Tren reach dan impressions menunjukkan pola yang fluktuatif sepanjang periode analisis. Puncak performa terjadi pada 4 September 2025 dengan 7.150 reach dan 25.419 impressions, sedangkan performa terendah terjadi pada 18 September 2025 dengan 150 reach dan 344 impressions. Kondisi ini mengindikasikan adanya variasi performa konten yang cukup signifikan dari hari ke hari.
- Distribusi engagement berdasarkan platform menunjukkan bahwa TikTok menjadi kontributor terbesar dengan porsi 33,1%, diikuti Instagram sebesar 28,7%, X sebesar 28,5%, dan Facebook sebesar 9,6%. Temuan ini menunjukkan bahwa sebagian besar interaksi pengguna berasal dari TikTok, Instagram, dan X.
- Berdasarkan content type, Story menjadi format dengan performa terbaik, menghasilkan total reach sebesar 19.500 dan total engagement sebesar 3.263. Sementara itu, Reel / Short Video, Carousel, dan Photo masing-masing menghasilkan reach sebesar 8.300, 6.600, dan 4.400.
- Berdasarkan content theme, Entertainment mencatat performa tertinggi dengan total reach sebesar 20.550 dan total engagement sebesar 3.715, diikuti Promotion dengan 7.950 reach dan 1.401 engagement, Educational dengan 6.100 reach dan 892 engagement, serta Behind the Scenes dengan 4.200 reach dan 732 engagement.
- Analisis berdasarkan tabel Social Media Content Performance menunjukkan bahwa kombinasi TikTok–Story–Entertainment menghasilkan engagement tertinggi sebesar 1.195, diikuti oleh Instagram–Story–Entertainment sebesar 669 dan X–Story–Entertainment sebesar 526. Temuan ini mengindikasikan bahwa konten bertema Entertainment dengan format Story cenderung memberikan performa interaksi yang lebih baik dibandingkan kombinasi konten lainnya.
